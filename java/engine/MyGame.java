...
public class MyGame extends PEGame
{
	...
	public void tick()
	{
		...
		if (this.powerup != null) {
			...
			if (this.powerupTimer == 0) {
				this.engine.remove(this.powerup);
				this.powerup = null;
			}
		} else {
			int rad = 0;
			if (player != null) {
				rad = this.player.getRadius();
			}
			if (rad == 10) {
				this.powerupTimer--;
				if (this.powerupTimer == 0) {
				...
				this.powerupTimer++;
				if (this.powerupTimer == 150) {
					this.powerup = this.factory.createShape();
					this.engine.add(powerup);
				}
			}
	...
	public void collisionDetected(List<PEWorldObject> objects)
	{
		...
		if ((worldObject1 instanceof Player || worldObject2 instanceof Player) && (worldObject1 instanceof Enemy || worldObject2 instanceof Enemy)) {
			...
			this.collisionStrategy.collide(p, e);
		} else if (((worldObject1 instanceof Player) && (worldObject2 instanceof PESquare)) || ((worldObject1 instanceof PESquare) && (worldObject2 instanceof Player))) {
			if (worldObject1 instanceof PESquare) {
				PESquare wo = (PESquare)worldObject1;
				int color = wo.getColor();
				if (color == 0xFF0000) {
					return;
				}
			} else {
				PESquare wo = (PESquare)worldObject2;
				int color = wo.getColor();
				if (color == 0xFF0000) {
					return;
				}
